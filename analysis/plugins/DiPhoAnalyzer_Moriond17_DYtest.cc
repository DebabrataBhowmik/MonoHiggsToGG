...
// curtesy of Y.Iiyama
typedef std::map<unsigned, std::set<unsigned> > Moriond17_EventList;   

Moriond17_EventList readMoriond17_EventList(char const* _fileName) {
  
  Moriond17_EventList list;   
  ifstream listFile;
...
class DiPhoAnalyzer_Moriond17 : public edm::EDAnalyzer {
  
public:
  
  explicit DiPhoAnalyzer_Moriond17(const edm::ParameterSet&);
  ~DiPhoAnalyzer_Moriond17();
...
DiPhoAnalyzer_Moriond17::DiPhoAnalyzer_Moriond17(const edm::ParameterSet& iConfig):
  // collections
...
DiPhoAnalyzer_Moriond17::~DiPhoAnalyzer_Moriond17() { 
...
int weight_pos,weight_neg;
 void DiPhoAnalyzer_Moriond17::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  for( unsigned index = 0; index < triggerNames.size(); ++index ) {
    ...
    if( (TString::Format((triggerNames.triggerName( index )).c_str())).Contains("HLT_Ele35_WPTight_Gsf_v")  )hltDiphoton30Mass95 = triggerBits->accept( index );
  ...
  for( unsigned index = 0; index < flashggtriggerNames.size(); ++index ){
    ...
    if (TString::Format((flashggtriggerNames.triggerName( index )).c_str())=="Flag_ecalBadCalibFilter"        && !triggerFlashgg->accept( index )) metF_EcalBadCalib = 0;
  }
  ...
  if (sampleID>0 && sampleID<10000) { 
    ...
    if(!eveWeights.empty()){
      ...
      if (gen_weight == 1) weight_pos += 1;
      if (gen_weight == -1) weight_neg += 1;
  ...
  */
  ...
  if (hltDiphoton30Mass95){
	...
	*/
	...
	float leadScaling  = c.ScaleCorrection(run,  leadIsEB, leadR9_SSRew, leadScEta, diphoPtr->leadingPhoton()->et());
        //float leadSigma    = c.getSmearingSigma(run, leadIsEB, leadR9_SSRew, leadScEta, diphoPtr->leadingPhoton()->et(), leadGain, 0, 0);
        float leadSigma    = c.getSmearingSigma(run, leadIsEB, leadR9_SSRew, leadScEta, diphoPtr->leadingPhoton()->et(), 0, 0);
	...
	*/
	...
	float subleadScaling  = c.ScaleCorrection(run,  subleadIsEB, subleadR9_SSRew, subleadScEta, diphoPtr->subLeadingPhoton()->et());
        float subleadSigma    = c.getSmearingSigma(run, subleadIsEB, subleadR9_SSRew, subleadScEta, diphoPtr->subLeadingPhoton()->et(), 0, 0);
	  ...
	  if (elvetoDipho.size()>0) {
		...
		if(vtxDipho.size()>0){
		  ...
		  for( size_t diphotonlooper = 0; diphotonlooper < vtxDipho.size(); diphotonlooper++ ) {  
		    ...
		    if (mapMggCorr[diphoPtr] < 60 || mapMggCorr[diphoPtr] > 300) continue;
			...
			if(abs(sceta2)<1.4442){
		      ...
		      */
		      ...
		      //float subleadScaling	= c.ScaleCorrection(run,subleadIsEB,r92,sceta2,pt2,subleadGain,bit_stat);
                      float leadScaling		= c.ScaleCorrection(run,leadIsEB,r91,sceta1,pt1);
		      float subleadScaling	= c.ScaleCorrection(run,subleadIsEB,r92,sceta2,pt2);
		      ...
		      float leadScaling_stat	= c.ScaleCorrectionUncertainty(run,leadIsEB,r91,sceta1,pt1);
		      float leadScaling_syst	= c.ScaleCorrectionUncertainty(run,leadIsEB,r91,sceta1,pt1);
		      ...
		      //float subleadScaling_stat	= c.ScaleCorrectionUncertainty(run,subleadIsEB,r92,sceta2,pt2,subleadGain,bit_stat);
                      ...
                      float subleadScaling_stat	= c.ScaleCorrectionUncertainty(run,subleadIsEB,r92,sceta2,pt2);
                      float subleadScaling_syst	= c.ScaleCorrectionUncertainty(run,subleadIsEB,r92,sceta2,pt2);
                      ///float subleadScaling_gain	= c.ScaleCorrectionUncertainty(run,subleadIsEB,r92,sceta2,pt2,subleadGain,bit_gain);

		      float leadScaling_total	= TMath::Sqrt(leadScaling_stat*leadScaling_stat + leadScaling_syst*leadScaling_syst);
                      float leadScalingUp	= leadScaling+leadScaling_total;
                      float leadScalingDown	= leadScaling-leadScaling_total; 
		      float subleadScaling_total= TMath::Sqrt(subleadScaling_stat*subleadScaling_stat + subleadScaling_syst*subleadScaling_syst);
		      ...
		      */
		      float leadSigma		= c.getSmearingSigma(run, leadIsEB,    r91, sceta1,  pt1, 0, 0);
		      float leadSigmaUp		= c.getSmearingSigma(run, leadIsEB,    r91, sceta1,  pt1, 0, 1);
		      float leadSigmaDown	= c.getSmearingSigma(run, leadIsEB,    r91, sceta1,  pt1, 0, -1);
                      float subleadSigma	= c.getSmearingSigma(run, subleadIsEB, r92, sceta2,  pt2, 0, 0);
                      float subleadSigmaUp	= c.getSmearingSigma(run, subleadIsEB, r92, sceta2,  pt2, 0, 1);
                      float subleadSigmaDown	= c.getSmearingSigma(run, subleadIsEB, r92, sceta2,  pt2, 0, -1);
  ...
  // delete
  //delete lazyToolnoZS;
  std::cout<<weight_pos << "   "<<weight_neg<<std::endl;
}
...
int DiPhoAnalyzer_Moriond17::sortedIndex(const unsigned int vtxIndex, const unsigned int sizemax, const Ptr<flashgg::DiPhotonCandidate> diphoPtr ) {
...
bool DiPhoAnalyzer_Moriond17::geometrical_acceptance(float eta1, float eta2)
{
  float ae1(fabs(eta1));
  float ae2(fabs(eta2));
...
void DiPhoAnalyzer_Moriond17::beginJob() {
...
void DiPhoAnalyzer_Moriond17::endJob() { 
...
void DiPhoAnalyzer_Moriond17::initTreeStructure() {
...
TGraph* DiPhoAnalyzer_Moriond17::GetSSRew_R9EB(std::string SSRewFile){
...
TGraph* DiPhoAnalyzer_Moriond17::GetSSRew_R9EE(std::string SSRewFile){
  TFile* fin_SSRew=new TFile(SSRewFile.c_str(), "READ");
  TGraph* g= (TGraph*) fin_SSRew->Get("transffull5x5R9EE");
  return g;
}
TGraph* DiPhoAnalyzer_Moriond17::GetSSRew_sieieEB(std::string SSRewFile){
  TFile* fin_SSRew=new TFile(SSRewFile.c_str(), "READ");
  TGraph* g= (TGraph*) fin_SSRew->Get("transffull5x5sieieEB");
  return g;
}
TGraph* DiPhoAnalyzer_Moriond17::GetSSRew_sieieEE(std::string SSRewFile){
  TFile* fin_SSRew=new TFile(SSRewFile.c_str(), "READ");
...
void DiPhoAnalyzer_Moriond17::SetPuWeights(std::string puWeightFile) {
...
float DiPhoAnalyzer_Moriond17::GetPUWeight(float pun) {
...
bool DiPhoAnalyzer_Moriond17::isGammaPresel( float sceta, float pt, float r9, float chiso, float hoe) {
...
bool DiPhoAnalyzer_Moriond17::rediscoveryHLT(float sceta, float pt, float r9,float sieie, float pfIso,float trkSum03 ){
  bool isEB=false;
  bool isEE=false;
  bool HLTok=false;
...
double DiPhoAnalyzer_Moriond17::getChargedHadronEAForPhotonIso(float eta){
...
double DiPhoAnalyzer_Moriond17::getNeutralHadronEAForPhotonIso(float eta) {
...
double DiPhoAnalyzer_Moriond17::getGammaEAForPhotonIso(float eta) {
...
bool DiPhoAnalyzer_Moriond17::LeadPhoTriggerSel(float eta, float hoe, float r9, float sieie, float phoiso, float pt){
  bool passes = false;
...
bool DiPhoAnalyzer_Moriond17::SubLeadPhoTriggerSel(float eta, float hoe, float r9, float sieie, float phoiso, float chiso, float pt){
  bool passes = false;
  if (fabs(eta)<1.4442 && hoe < 0.12){
...
int DiPhoAnalyzer_Moriond17::passSieieCuts(float sceta, float sieie){
  int passes = 1;
  if (fabs(sceta)<1.4442 && sieie>0.0103) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && sieie>0.0271) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passCHisoCuts(float sceta, float chiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && chiso>1.416) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && chiso>1.012) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passNHisoCuts(float sceta, float nhiso, float pt){
  int passes = 1;
...
int DiPhoAnalyzer_Moriond17::passPHisoCuts(float sceta, float phiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && phiso > (2.952 + 0.0035*pt)) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && phiso > (4.059 + 0.0040*pt)) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passHoeCuts(float sceta, float hoe){
  int passes = 1;
...
// tight working point
int DiPhoAnalyzer_Moriond17::passTightSieieCuts(float sceta, float sieie){
  int passes = 1;
  if (fabs(sceta)<1.4442 && sieie>0.0103) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && sieie>0.0271) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passTightCHisoCuts(float sceta, float chiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && chiso>1.158) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && chiso>0.575) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passTightNHisoCuts(float sceta, float nhiso, float pt){
  int passes = 1;
...
int DiPhoAnalyzer_Moriond17::passTightPHisoCuts(float sceta, float phiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && phiso > (2.065 + 0.0035*pt)) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) &&  phiso > (3.272 + 0.0040*pt)) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passTightHoeCuts(float sceta, float hoe){
  int passes = 1;
...
// loose working point
int DiPhoAnalyzer_Moriond17::passLooseSieieCuts(float sceta, float sieie){
  int passes = 1;
  if (fabs(sceta)<1.4442 && sieie>0.0106) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && sieie>0.0272) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passLooseCHisoCuts(float sceta, float chiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && chiso>1.694) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) && chiso>2.089) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passLooseNHisoCuts(float sceta, float nhiso, float pt){
  int passes = 1;
...
int DiPhoAnalyzer_Moriond17::passLoosePHisoCuts(float sceta, float phiso, float pt){
  int passes = 1;
  if (fabs(sceta)<1.4442 && phiso > (2.876 + 0.004017*pt)) passes = 0;
  if ((fabs(sceta)>1.566 && fabs(sceta)<2.5) &&  phiso > (4.162 + 0.0037*pt)) passes = 0;
  return passes;
}
int DiPhoAnalyzer_Moriond17::passLooseHoeCuts(float sceta, float hoe){
  int passes = 1;
...
bool DiPhoAnalyzer_Moriond17::testPhotonIsolation(int passSieie, int passCHiso, int passNHiso, int passPHiso, int passHoe, int passEleVeto){
  if (passSieie == 1 && passHoe == 1 && passCHiso == 1 && passNHiso == 1 && passPHiso == 1 && passEleVeto == 1) return true; //passes all selection
  else return false;
}


int DiPhoAnalyzer_Moriond17::passPHisoCorr(double phoiso1, double phi1, double phi2, double eta1, double eta2, double pt1, double pt2, int isLeading){
...
bool DiPhoAnalyzer_Moriond17::passHighPtSelection(float rho, float pt, float sceta, float hoe, float sieie, float chiso, float phoiso){
  bool passes = false;
...
bool DiPhoAnalyzer_Moriond17::isGammaSelected( float rho, float pt, float sceta, float r9, float chiso, float nhiso, float phoiso, float hoe, float sieie, bool passElectronVeto) {
...
int DiPhoAnalyzer_Moriond17::effectiveAreaRegion(float sceta) {
...
float DiPhoAnalyzer_Moriond17::getPhoSFValue(float eta, float pt,double dr){
  ...
  if (-2.5   <= eta && eta < -2.0  ){
    if (pt <= 35) SF = 1.041;
    if (35 < pt && pt <= 50)  SF = 0.995;
    if (50 < pt && pt <= 100)  SF = 0.978;
    if (100 < pt && pt <= 200) SF = 0.983;
    if (pt > 200) SF = 0.998;
  }
  if (-2.0   <= eta && eta < -1.566){
    if (pt <= 35) SF = 0.986;
    if (35 < pt && pt <= 50)  SF = 0.978;
    if (50 < pt && pt <= 100)  SF = 0.982;
    if (100 < pt && pt <= 200) SF = 0.996;
    if (pt > 200) SF = 1.018;
  }
  if (-1.444 <= eta && eta < -0.8  ){
    if (pt <= 35) SF = 1.026;
    if (35 < pt && pt <= 50)  SF = 0.998;
    if (50 < pt && pt <= 100)  SF = 0.985;
    if (100 < pt && pt <= 200) SF = 0.981;
    if (pt > 200) SF = 0.971;
  }
  if (-0.8   <= eta && eta < 0.0   ){
    if (pt <= 35) SF = 1.018;
    if (35 < pt && pt <= 50)  SF = 0.989;
  ...
  if (0.0    <= eta && eta < 0.8   ){
    ...
    if (50 < pt && pt <= 100)  SF = 0.979;
    if (100 < pt && pt <= 200) SF = 0.997;
    if (pt > 200) SF = 0.968;
  }
...
float DiPhoAnalyzer_Moriond17::getPhoTRIGGERSFValueLeadPhoton(float r9,float eta, float pt){
...
float DiPhoAnalyzer_Moriond17::getPhoTRIGGERSFValueSubLeadPhoton(float r9,float eta, float pt){
...
float DiPhoAnalyzer_Moriond17::getSmearingValue(float sceta, float r9, int syst){
...
float DiPhoAnalyzer_Moriond17::getScalingValue(int sampleID, float sceta, float r9, int runNumber, int syst){
...
float DiPhoAnalyzer_Moriond17::applyGainCorr(float ptUncorr, float sceta, float energy){ 
...
float DiPhoAnalyzer_Moriond17::applyEnergySmearing(float ptUncorr, float sceta, float r9, unsigned int event, int syst){
...
float DiPhoAnalyzer_Moriond17::applyEnergyScaling(int sampleID, float ptUncorr, float sceta, float r9, int run, int syst){
  float Scaling	= getScalingValue(sampleID, sceta, r9, run, syst);
  float pt = ptUncorr*Scaling; 
  return pt;
}

float DiPhoAnalyzer_Moriond17::getPtCorrected(float ptUncorr, float sceta, float r9, int run, int sampleID, float energy,unsigned  int event, int syst){
  float ptScaled;
  float pt;
...
DEFINE_FWK_MODULE(DiPhoAnalyzer_Moriond17);
